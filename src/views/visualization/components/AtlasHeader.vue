<template>
  <div :class="['dark', 'atlas-header', 'between-flex--1024px']">
    <div class="center-flex">
      <span class="title p-r-16"> {{ atlas.title }} </span>
      <el-tooltip v-if="atlas.url">
        <template #content>
          <strong class="p-r-8">Atlas website:</strong>
          <span>{{ atlas.url }}</span>
        </template>
        <el-link :href="atlas.url" icon="Link" target="_blank" style="font-size:1.25rem"/>
      </el-tooltip>
    </div>
    <div class="toolbar center-flex">
      <div style="text-align: center" v-if="layoutChange" >
        <el-dropdown v-if="subpages.length || (breakpoint !== 'lg' && breakpoint !== 'xl')" @command="handleSetLayout">
      ...
      <el-radio-group v-model="curPage" v-if="subpages.length">
        <el-radio-button v-for="p in subpages" :label="p.id" :key="p.name">
          <span> {{  p.title  }} </span>
        </el-radio-button>
      </el-radio-group>
      <div>
...
const emit = defineEmits(["layoutChange", "update:subpage"]);
...
const showRenderSetting = true
const showRenderData = true

const atlas = inject("atlas");
