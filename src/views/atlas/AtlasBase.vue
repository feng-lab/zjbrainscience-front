  ...
  <el-row class="atlas-content">
    <el-col :span="leftSpan" class="atlas-content-left">
      <slot name="left">
        ...
        <bs-region-tooltip v-if="focusRegion?.id" :focus-region="focusRegion" />
        <bs-widgets
          header="Connectivity"
          v-if="focusConnectivity?.id"
          style="flex-grow: 1"
        >
          <p class="between-flex">
            <span class="conn-label">
              ...
              <bs-icon-img
                icon="Right"
                v-if="focusConnectivity.opposite_value"
              />
              <bs-icon-img icon="Switch" v-else />
              <span> {{ focusConnectivity.region2 }} </span>
            </span>
            <span class="m-l-8 red" v-if="focusConnectivity.value">{{
              Number(focusConnectivity.value).toExponential()
            }}</span>
          </p>
          <p v-if="focusConnectivity.opposite_value" class="between-flex">
            ...
            <span class="m-l-8 red">{{
              Number(focusConnectivity.opposite_value).toExponential()
            }}</span>
          </p>
        </bs-widgets>
      </slot>
    ...
    <el-col
      :span="neuroSpan"
      class="atlas-content-neuro neuroglancer p-l-8 p-r-8"
    >
      <slot name="neuro">
        <vue-neuroglancer
    ...
    <el-col :span="chartSpan" class="atlas-content-right">
      <slot name="right">
        <div
          v-for="chart in Object.keys(renderDatas).filter(
            (rd) => renderDatas[rd].type === 'chart'
          )"
          :key="chart"
        >
          <bs-widgets
            :header="renderDatas[chart].label"
            v-if="renderDatas[chart].show"
          >
            <slot :name="chart" />
          </bs-widgets>
        </div>
    ...
    <el-col
      ...
      :sm="5"
      :lg="4"
      class="atlas-content-setting p-l-8"
...
const showRegionLabel =
  props.neuroglancerDatas.layers.filter(
    (d) => d.type === 'segmentation' && !d?.notShowLabelTree
  ).length > 0
...
const showSettingPanel = computed(
  () => showHelper.value || showRenderSetting.value || showRenderData.value
)

const chartSpan = computed(() => {
  if (
    Object.values(renderDatas.value).filter(
      (rd) => rd.type === 'chart' && rd.show
    ).length ||
    props.rightCustom
  ) {
...
const { regionLayout, getAtlasLayout } = useBrainAtlasLayout(
  props.neuroglancerDatas.layout ? props.neuroglancerDatas.layout : '4panel'
)

const { treeRef, regionsLabelTree, filterText, checkTreeNode, filterTreeNode } =
  useRegionsLabelTree(neuroRef, atlasInfo, props.atlasId, showRegionLabel)

const { selectRegion, handleSelectRegion } = useRegionHandler(props.atlasId)
...
const clickEventsHandler = {
  ...
  regions: async (id) =>
    await handleSelectRegion(id, renderDatas, neuroRef, updateState),
}
...
console.log('focusRegion--->666', focusRegion)
...
const handleDataVisibleChange = (renderData, isRender) => {
  ...
  if (regionRelated && isRender) {
    nextTick(async () => {
      await handleSelectRegion(
        selectRegion.value?.region_id ?? '1',
        renderDatas,
        neuroRef
      )
    })
  ...
  if (type === 'neuroglancer') {
    ...
    if (renderData?.connectivityType === 'lr') {
      const regionLayer = neuroRef.value.getManagedLayer(
        `${atlasInfo.value.name}_regions`
      )
      regionLayer.layer.displayState.silhouetteRendering.value = Number(
        !!isRender
      )
    ...
    if (name.endsWith('_fc') || name.endsWith('_sc')) {
      ...
      const projectionScale = notGroupViewr
        ? _projectionScale
        : _projectionScale * 1.3
      const crossSectionScale = notGroupViewr
        ? _crossSectionScale
        : _crossSectionScale * 1.3
...
onMounted(() => {
  ...
  if (props.neuroglancerDatas && props.neuroglancerDatas.layers) {
    if (props.chartDatas && props.chartDatas.layers) {
      showRelatedRegionInfoDefault = [
        ...props.neuroglancerDatas.layers,
        ...props.chartDatas.layers,
      ].some((rd) => rd.regionRelated && !rd?.defaultHidden)
    } else {
      showRelatedRegionInfoDefault = props.neuroglancerDatas.layers.some(
        (rd) => rd.regionRelated && !rd?.defaultHidden
      )
    }
  }
